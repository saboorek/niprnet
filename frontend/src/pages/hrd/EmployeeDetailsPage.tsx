...
export const EmployeeDetailsPage = () => {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const resAvatar = await fetch(`${config.URL}/characters/${characterId}/avatar`, {
                method: 'PUT',
                credentials: 'include',
                headers: { 'Content-Type': 'application/json' },
            ...
            if (resEmployee.ok && resAvatar.ok) {
                toast.success('Zaktualizowano teczkę pracownika');
                setEmployee(prev => prev ? { ...prev, avatarUrl, phone, notes: notesList, ribbons, squadronId, sectionId } : null);
            } else {
    ...
    const handleAddNoteEntry = async (e: React.FormEvent) => {
        ...
        if (modalType === 'promotion') {
            ...
            finalContent = `${employee.firstName} ${employee.lastName} została awansowana na stopień ${selectedRank}`;
        } else if (modalType === 'demotion') {
            if (!selectedRank) {
                toast.error('Wybierz nowy stopień po degradacji');
                return;
            }
            finalContent = `${employee.firstName} ${employee.lastName} została zdegradowana na stopień ${selectedRank}`;
        } else {
            if (!noteContent.trim()) {
                ...
                <div className="bg-stone-900 border border-stone-800 rounded-xl p-6 flex flex-col items-center gap-4">
                    ...
                    {canManage && (
                        <div className="w-full">
                            ...
                            <input
                                ...
                                placeholder="https://..."
                                className="w-full bg-stone-800 border border-stone-700 text-white text-xs rounded-lg px-3 py-2 outline-none focus:border-stone-500"
                            />
                        </div>
                    )}
                    ...
                    <div className="w-full">
                        ...
                        <input
                            ...
                            className="w-full bg-stone-800 border border-stone-700 text-white rounded-lg px-3 py-2 text-sm outline-none focus:border-stone-500 disabled:opacity-50"
                        />
                    </div>
                </div>
                    ...
                    {canManage && (
                        <div className="flex justify-end pt-4 border-t border-stone-800 mt-4">
                            ...
                            >
                                ...
                                {saving ? 'Zapisywanie...' : 'Zapisz Teczkę'}
                            </button>
                        </div>
                    )}
                </div>
